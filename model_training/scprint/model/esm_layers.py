...
ESMMultiheadAttention = MultiheadAttention
...
class FusionLayer(nn.Module):
    def __init__(self, d_model=768, nhead=12, dropout=0.1):
        ...
        self.structural_adapter_ffn = NormalizedResidualBlock(
            layer=FeedForwardNetwork(
                d_model,
                # TODO: @mf: Why is it important?
                d_model // 2,  # NOTE: bottleneck FFN is important
                # self.ffn_embed_dim,
    ...
    def forward(
        self,
        src_total_embs: torch.Tensor,
        src_key_padding_mask: Optional[torch.Tensor] = None,
        structure_feature: Optional[torch.Tensor] = None,
    ):
        if (
            src_key_padding_mask is not None
            and src_key_padding_mask.dtype != torch.bool
        ):
            src_key_padding_mask = src_key_padding_mask.bool()

        # switch to batch second
        src_total_embs = src_total_embs.transpose(0, 1)
        structure_feature = structure_feature.transpose(0, 1)

        padding_mask = src_key_padding_mask

        src_delta_embs = self.structural_adapter_attn(
            src_total_embs,
            key=structure_feature,
            value=structure_feature,
            key_padding_mask=padding_mask,
            attn_mask=None,
            need_weights=False,
        )[0]
        src_delta_embs = self.structural_adapter_ffn(src_delta_embs)

        # switch back to batch first
        src_delta_embs = src_delta_embs.transpose(0, 1)

        return src_delta_embs
