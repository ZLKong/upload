...
class Dataset(torchDataset):
    ...
    extra_obs: Optional[list[str]] = field(
        default_factory=[]
    )  # extra obs will not be encoded to integers
    ...
    def __post_init__(self):
        ...
        self.image_preprocesser = CLIPProcessor.from_pretrained(
            self.clip_model_type
        ).image_processor

        # Initialize environment list for LMDB files
        self.env_list = []
        if self.spatial_datadir is not None:
            ...
            # 如果没有成功打开任何LMDB环境，设置spatial_datadir为None
    ...
    def __getitem__(self, *args, **kwargs):
        ...
        if self.spatial_datadir is not None and self.env_list:
            ...
            # Try to load image from each LMDB environment in order
            for i, env in enumerate(self.env_list):
                ...
                with env.begin() as txn:
                    try:
                        ...
                        # If image retrieval fails, use zero tensor
