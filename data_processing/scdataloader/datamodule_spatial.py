...
from .collator_spatial import Collator
from .data_spatial import Dataset
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            spatial_datadir (Union[str, list[str]], optional): Path(s) to LMDB file(s). Can be a single path, a comma-separated string, or a list. Defaults to None.
            **kwargs: Additional keyword arguments passed to the pytorch DataLoader.

            see @file data.py and @file collator.py for more details about some of the parameters
        """
        if collection_name is not None:
            # 处理spatial_datadir参数，支持逗号分隔的字符串格式
            if spatial_datadir is not None and isinstance(spatial_datadir, str) and "," in spatial_datadir:
                spatial_datadir = [path.strip() for path in spatial_datadir.split(",")]
                print(f"将逗号分隔的空间数据路径转换为列表: {spatial_datadir}")
